...
//! instead of handle_all_messages() which loops forever when reports are streaming.

use bno08x::{
    BNO08x, SENSOR_REPORTID_ACCELEROMETER, SENSOR_REPORTID_GRAVITY, SENSOR_REPORTID_GYROSCOPE,
    SENSOR_REPORTID_LINEAR_ACCEL, SENSOR_REPORTID_MAGNETIC_FIELD, SENSOR_REPORTID_ROTATION_VECTOR,
...
fn test_accelerometer() {
    ...
    assert!(magnitude > 8.0 && magnitude < 12.0,
            "Accelerometer magnitude {} outside expected range", magnitude);
...
fn test_gyroscope() {
    ...
    imu.enable_gyro(REPORT_INTERVAL_MS).expect("Failed to enable gyroscope");
    sleep(Duration::from_millis(SENSOR_WARMUP_MS));
    ...
    assert!(gyro[0].abs() < 1.0 && gyro[1].abs() < 1.0 && gyro[2].abs() < 1.0,
            "Gyroscope readings {:?} too high for stationary sensor", gyro);
...
fn test_magnetometer() {
    ...
    assert!(magnitude > 10.0 && magnitude < 100.0,
            "Magnetic field {} outside expected range", magnitude);
...
fn test_rotation_vector() {
    ...
    assert!((magnitude - 1.0).abs() < 0.1,
            "Quaternion magnitude {} should be close to 1.0", magnitude);
    
    println!("✓ Rotation: {:?}, |q| = {:.3}, acc = {:.3} rad", quat, magnitude, acc);
}
...
fn test_linear_acceleration_and_gravity() {
    ...
    imu.enable_linear_accel(REPORT_INTERVAL_MS).expect("Failed to enable linear accel");
    imu.enable_gravity(REPORT_INTERVAL_MS).expect("Failed to enable gravity");
    ...
    assert!(linear_mag < 2.0, "Linear accel too high for stationary sensor");
    
    // Gravity should be ~9.8 m/s²
    assert!(gravity_mag > 8.0 && gravity_mag < 11.0,
            "Gravity magnitude {} outside expected range", gravity_mag);
    
    println!("✓ Linear accel: {:?}, |a_lin| = {:.2} m/s²", linear, linear_mag);
...
fn test_handle_messages_with_limit() {
    ...
    // This is how the working imu application does it: handle_messages(timeout, max_count)
    let count = imu.handle_messages(REPORT_INTERVAL_MS as usize * 2, 5);
    
    assert!(count > 0 && count <= 5, "Should have handled 1-5 messages, got {}", count);
...
fn test_sensor_report_callback() {
    ...
    assert!(count > 0, "Callback should have been triggered, got {}", count);
...
fn test_report_status_queries() {
    ...
    assert!(update_time > 0, "Update time should be non-zero after receiving data");
...
fn test_multiple_sensors() {
    ...
    imu.enable_report(SENSOR_REPORTID_ACCELEROMETER, REPORT_INTERVAL_MS).expect("accel");
    imu.enable_gyro(REPORT_INTERVAL_MS).expect("gyro");
    imu.enable_rotation_vector(REPORT_INTERVAL_MS).expect("rotation");
