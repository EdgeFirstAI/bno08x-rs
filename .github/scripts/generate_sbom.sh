...
# Last synchronized with policy version: 2.1.1 (2025-11-28)
#
...
#   3. Generating Rust dependencies with cargo-cyclonedx
#   4. Merging all SBOMs into a single file
#   5. Validating license policy compliance
#   6. Validating NOTICE file (if present)
#
...
if [ -f "Cargo.toml" ]; then
    ...
    # cargo-cyclonedx creates files named {crate-name}.cdx.json
    if [ -f "${PROJECT_NAME}.cdx.json" ]; then
        cp "${PROJECT_NAME}.cdx.json" deps-sbom.json
        echo "  ✓ Generated Rust dependency SBOM from ${PROJECT_NAME}.cdx.json"
    else
        # Fallback: find and use any .cdx.json file
        CDX_FILES=$(find . -maxdepth 1 -name "*.cdx.json" -type f 2>/dev/null)
        if [ -z "$CDX_FILES" ]; then
...
rm -f $SBOM_FILES source-sbom.json deps-sbom.json sbom-deps.json *.cdx.json
